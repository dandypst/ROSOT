...
export default function ScorePage({ user }) {
  ...
  useEffect(() => {
    const load = async () => {
      const q = query(collection(db, "answer_records"), where("userId", "==", user.uid), orderBy("timestamp", "desc"));
  ...
  if (loading) return <div style={{ color: "#475569", padding: 40 }}>Memuat skor…</div>;
  ...
  return (
    <div style={S.pageWrap}>
        ...
        {[
          ...
          <div key={i} style={{ marginBottom: 16 }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 6 }}>
              <span style={{ color: "#cbd5e1", fontSize: 14 }}>{b.label}</span>
              <span style={{ color: b.color, fontSize: 14, fontWeight: 600 }}>{b.pct}% <span style={{ color: "#475569", fontWeight: 400 }}>({b.total} soal)</span></span>
            </div>
      ...
      <div style={S.card}>
        ...
        {records.length === 0 && <div style={{ color: "#64748b", fontSize: 14 }}>Belum ada jawaban. Mulai dari Soal Harian!</div>}
        {records.map((r, i) => (
          <div key={i} style={{ borderBottom: "1px solid #1e293b", padding: "12px 0", display: "flex", gap: 12, alignItems: "flex-start" }}>
            <span style={{ fontSize: 18, flexShrink: 0 }}>{r.timeout ? "⏰" : r.correct ? "✅" : "❌"}</span>
            <div style={{ flex: 1 }}>
              <div style={{ color: "#cbd5e1", fontSize: 13, marginBottom: 4 }}>{r.questionText}</div>
              ...
              {!r.correct && !r.timeout && (
                <div style={{ fontSize: 12, color: "#64748b", marginTop: 4 }}>
                  Jawabanmu: <span style={{ color: "#f87171" }}>{r.chosenText}</span> · Benar: <span style={{ color: "#4ade80" }}>{r.correctAnswer}</span>
                </div>
              )}
            </div>
          </div>
